...
$$aPb \text{ iff } \matrix(\phi^+(a)>\phi^+(b) \text{ & } \phi^-(a)\leq\phi^-(b) \phi^+(a)\geq\phi^+(b) \text{ & } \phi^-(a)<\phi^-(b)$$

$$aPb \text{ iff }\phi^+(a)>\phi^+(b) \text{ & } \phi^-(a)\leq\phi^-(b)$$
...
$$\phi(a) = \phi^+(a) - \phi^-(a)$$
- $\phi(a)$ is the net outranking flow, with the following properties
$$-1 \leq \phi(a) \leq 1$$
...
$\phi(a) < 0 \implies$ a is more outranked by other alternatives
...
- $\phi(a)$ relies on comparative statements rather than absolute statements
