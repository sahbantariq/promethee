...
gaia_plot <- function(data, beneficial_col,
  ...
  (plot2 <- ggplot() +
      ggplot2::geom_point(data = scores1, aes(PC1, PC2)) +
      ggplot2::geom_text(data = scores1, colour = "red",
                         alpha = 0.8, size = 4, aes(
                           PC1,PC2, label = rownames(scores1)), hjust = 0) +
      ggplot2::geom_point(data = scores2, aes(PC1, PC2)) +
      ggplot2::geom_text(data = scores2, colour = "blue",
                         alpha = 0.8, size = 4, aes(
                           PC1,PC2, label = rownames(scores2)), hjust = 0) +
